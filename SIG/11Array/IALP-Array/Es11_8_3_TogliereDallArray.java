/*
11.8.3 Togliere dall’array
Come l’esercizio precedente ma lo scopo sarà quello di togliere la cella indicizzata 
dall’utente
(vedi pdf)
*/
...
public class Es11_8_3_TogliereDallArray {
    ...
    public static void main(String[] args) {
        ...
        int origine;
        int destinazione = -100;// -100 e' un valore fittizio per poter inizzializzare la variable*
        ...
        for(origine = 0; origine < array.length; origine++) {   // scorriamo tutti gli elementi dell'array di origine
            ...
            if(origine < cella) {                               // fino alla cella indicata dall'utente

                destinazione = origine;                         // copiamo pari-pari
            } else if (origine > cella) {                       // altrimenti, saltiamo la cella stessa
        
                destinazione = origine-1;                       // calcoliamo la nuovo posizione dell'array (-1) 
            }

            if(origine != cella) {

                tmp[destinazione] = array[origine];             // copiamo l'elemento con gli indici giusti
                                                                // inizzializzazione variabile destinazione richiesta*
            }

        }

        // se non uso tmp[cella] = valore; l'array verra' diminuito di 1 spostando i valori
        tmp[cella] = valore;                                    // inseriamo il nuovo valore (input utente) nella
                                                                // prima posizione
